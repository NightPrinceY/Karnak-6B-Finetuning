...
ALTQ_AUDIO = ["شغّل لي سورة {alt}.", "أسمعني سورة {alt}."]
# v5 fix: the "تبارك" alt-name failure (eval probe v4_surah_mulk_altname --
# model resolved "شغّل لي سورة تبارك" to the wrong surah, 108 instead of 67)
# traced back to a single-point-of-failure: with only ONE randomly-drawn
# phrasing per name/tool-path, the sole play_surah-path row for تبارك landed
# entirely in the val split by chance, so the model never saw that specific
# tool-path for that specific name in training. Fix: emit EVERY phrasing
# variant for EVERY name/tool-path (not one random draw each) so no single
# name's coverage of a given tool depends on a single row surviving one
# random split.
for alt, s in ALT_SURAH_NAMES:
    for tmpl in ALTQ:
        user = tmpl.format(alt=alt)
    ...
    # audio-tool-path phrasings, exercising the SAME name->number resolution
    # on play_surah -- the exact path that failed for تبارك
    for tmpl in ALTQ_AUDIO:
        user2 = tmpl.format(alt=alt)
        rec = random.choice(SURAH_RECITERS)
...
outdir = HERE
with open(outdir / "muslim_lora_train_v5.jsonl", "w", encoding="utf-8") as f:
    for d in train:
        f.write(json.dumps(d, ensure_ascii=False) + "\n")
with open(outdir / "muslim_lora_val_v5.jsonl", "w", encoding="utf-8") as f:
    for d in val:
        f.write(json.dumps(d, ensure_ascii=False) + "\n")
